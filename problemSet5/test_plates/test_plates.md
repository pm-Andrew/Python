...
{% video https://www.youtube.com/watch?v=mQZmCJUSC6g %}

In a file called `plates.py`, reimplement [Vanity Plates](/psets/2/plates/) from [Problem Set 2](/psets/2/), restructuring your code per the below, wherein `is_valid` still expects a `str` as input and returns `True` if that `str` meets all requirements and `False` if it does not, but `main` is only called if the value of `__name__` is `"__main__"`:
